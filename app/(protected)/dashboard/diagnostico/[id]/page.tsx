...
import { ArrowLeft, Edit3, Gamepad2, Crown, Target, Trophy, Clock, Star, AlertCircle } from "lucide-react"
import { getDiagnosticTestById, DiagnosticTest, submitDiagnosticTestAnswers, RespostaExercicio, SubmitTestResponse } from "@/services/diagnosticService"
...
interface DiagnosticTestLocal extends Omit<DiagnosticTest, 'competencias'> {
...
export default function DiagnosticViewPage() {
                    ...
                    competencias: testResponse.competencias?.map(compApi => ({
                        ...
                        exercicios: compApi.exercicios?.map(exApi => ({
                            ...
                            opcoes: exApi.opcoes?.map(opApi => ({
                                id: generateId(), // GERA um ID para a opção para o frontend
    ...
    const handleSubmit = async () => {
        ...
        const respostasFormatadas: RespostaExercicio[] = todosExercicios.map(ex => {
            ...
            return {
                exercicio_id: ex.id,
                resposta: ex.jogabilidade === "MULTIPLA_ESCOLHA"
                    ? respostaSelecionada
                    ...
                    <CardContent className="p-8 text-center">
                        ...
                        <div className="flex items-center justify-center gap-4">
                            <Badge variant="secondary" className="bg-[#f9d570]/20 text-[#172750] hover:bg-[#f9d570]/20">
                                {testeData.nome_classe || classes.find(c => c.id === testeData.classe_id)?.name || "N/A"}
                            </Badge>
                            <Badge variant="secondary" className="bg-[#172750]/10 text-[#172750] hover:bg-[#172750]/10">
                                {testeData.nome_disciplina || disciplinas.find(d => d.id === testeData.disciplina_id)?.disciplina_base.nome || "N/A"}
                            </Badge>
                        </div>
                    </CardContent>
                </Card>
                        ...
                        <CardContent className="p-8">
                            ...
                            <div className="space-y-3">
                                ...
                                {exercicio.opcoes?.map((opcao, index) => (
                                    <div
                                        key={opcao.id}
                                        className={`p-4 rounded-lg border-2 cursor-pointer transition-all ${respostas[exercicio.id] === opcao.id
                                        ...
                                        <div className="flex items-center gap-3">
                                            {exercicio.jogabilidade === "MULTIPLA_ESCOLHA" ? (
                                                <input
                                                    ...
                                                    value={opcao.id}
                                                    checked={respostas[exercicio.id] === opcao.id}
                                                ...
                                                <input
                                                    type="radio"
                                                    name={`exercicio-${exercicio.id}`}
                                                    value={opcao.texto_opcao.toLowerCase()}
                                                    checked={respostas[exercicio.id] === opcao.texto_opcao.toLowerCase()}
                                                    onChange={() => selecionarResposta(exercicio.id, opcao.texto_opcao.toLowerCase())}
                                                    className="form-radio h-5 w-5 text-[#f39d15]"
                                                />
                                            )}
                            ...
                            <div className="flex justify-between mt-8">
                                ...
                                {exercicioAtual === todosExercicios.length - 1 ? (
                                    <Button
                                        ...
                                        disabled={isSubmitting || !alunoId || Object.keys(respostas).length !== todosExercicios.length}
